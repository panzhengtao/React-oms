...
class Condition extends Component {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                for (let i in values) {
                    if (values[i]) {
                        if (i == 'searchplatformkeyword') {
                            newobj[i] = values[i]
                        }
                    }
                }
                console.log(newobj)
                this.props.fetchsearchplatform({key: 'data', value: newobj})
    ...
    render() {
        ...
        const taglist = this.props.platformsearch.data ? this.props.platformsearch.data.map((v,i) => {
                ...
                <Row style={{'padding': '8px 0px'}}>
                    <Col span={24}>
                        ...
                        >
                            ...
                            {getFieldDecorator('searchplatformkeyword', {
                                rules: [{
                                    required: false,
                                    message: `搜索条件`
                                }],
